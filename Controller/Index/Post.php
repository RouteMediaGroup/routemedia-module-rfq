...
class Post extends \RouteMedia\Rfq\Controller\Index
{
        
        const CONFIG_CAPTCHA_ENABLE = 'rfq/google_options/captchastatus';
        const CONFIG_CAPTCHA_PRIVATE_KEY = 'rfq/google_options/googleprivatekey';
...
/*Save Data Start*/
                ...
                $model = $this->_objectManager->create('RouteMedia\Rfq\Model\Rfq');
                $model->setData($post);
    ...
    private function _processFileUpload()
    {
        ...
        try {
            ...
            if (isset($result) && !empty($result['name'])) {
                ...
                $rfq_dir = $media_dir.'/Rfq/';
                                
                                
                        $Uploader->save($rfq_dir);
                        return 'Rfq/'.$Uploader->getUploadedFileName();
            }
        } catch (\Exception $e) {
                    $this->messageManager->addError(
                        __($e->getMessage())
